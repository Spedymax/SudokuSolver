...
def find_board(img):
    """Takes an image as input and finds a sudoku board inside the image"""
...
def displayNumbers(img, numbers, color=(0, 255, 0)):
    ...
    for i in range (9):
        for j in range (9):
            if numbers[(j*9)+i] !=0:
                cv2.putText(img, str(numbers[(j*9)+i]), (i*W+int(W/2)-int((W/2)), int((j)*H)), cv2.FONT_HERSHEY_COMPLEX, 2, color, 2, cv2.LINE_AA)
    return img
